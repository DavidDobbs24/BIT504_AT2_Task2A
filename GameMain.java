...
public class GameMain extends JPanel implements MouseListener {
    ...
    /** After each turn check if the current player has won by putting their symbol in that position,
     * If they have, the GameState is set to won for that player.
     * If no winner, then isDraw is called to see if it's a draw, if not, GameState stays as PLAYING.
     */
    public void updateGame(Player thePlayer, int row, int col) {
        // Check for win after play
        if (board.hasWon(thePlayer, row, col)) {
            // Check which player has won and update the current state
            currentState = (thePlayer == Player.Cross) ? GameState.Cross_won : GameState.Nought_won;
        } else if (board.isDraw()) {
            // No more empty cells, it's a draw
     ...
     * If the selected cell is valid and Empty then the current player is added to cell content.
     * UpdateGame is called which will call the methods to check for a winner or Draw.
     * If none, then GameState remains playing.
     * If win or Draw then call is made to method that resets the game board.
     * Finally, a call is made to refresh the canvas so that the new symbol appears.
     */
